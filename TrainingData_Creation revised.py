...
folder_creation("TrainningDataFolder")
folder_creation("TrainningDataFolder/hit")
folder_creation("TrainningDataFolder/miss")
...
while(True):
	...
	if( Shift != 0):
		cv2.imwrite("TrainningDataFolder/hit/"+str(time.time())+".png",cv2.cvtColor( np.array(screengrab),cv2.COLOR_BGR2RGB))
	elif (Ctrl != 0):
		cv2.imwrite("TrainningDataFolder/miss/"+str(time.time())+".png",cv2.cvtColor( np.array(screengrab),cv2.COLOR_BGR2RGB))
